...
export function TrialCampaignsModal({ planId, planVersionModuleId, moduleName, isFreePlan, onClose }: TrialCampaignsModalProps) {
  ...
  const { data: allCampaigns = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await adminApi.get<TrialCampaign[]>(`/api/v1/admin/trial-campaigns/by-plan/${planId}`)
      return data
    },
  })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Record<string, unknown> }) =>
      adminApi.put(`/api/v1/admin/trial-campaigns/${id}`, payload),
    onSuccess: () => {
      invalidate()
      setEditingId(null)
      setError(null)
  ...
  const closeMutation = useMutation({
    mutationFn: (id: string) => adminApi.post(`/api/v1/admin/trial-campaigns/${id}/close`, {}),
