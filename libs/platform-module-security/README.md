...
```
-e ../libs/platform-module-security
```

Quando um indice PyPI privado estiver disponivel, a mesma dependencia passa a ser:

```
pip install platform-module-security
```

ou, direto do repositorio:

```
pip install git+ssh://git@<host>/<org>/<repo>.git#subdirectory=libs/platform-module-security
```
...
Todas as excecoes da biblioteca (`platform_security.exceptions`) sao subclasses de
