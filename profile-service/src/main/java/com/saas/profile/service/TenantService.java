...
import com.saas.profile.repository.TenantRepository;
...
public class TenantService {
    ...
    public Tenant createTenant(String name, String slug, UUID ownerId, String type) {
        ...
        UUID freePlanId = tenantRepository.findPlanIdByType(type)
                .or(tenantRepository::findAnyActivePlanId)
        ...
        tenantRepository.insertOwnerLink(ownerId, tenant.id);
        tenantRepository.insertSubscription(tenant.id, freePlanId);
    ...
    public TenantProfileResponse getTenantProfile(UUID tenantId) {
        var row = tenantRepository.findTenantProfile(tenantId)
    ...
    private void ensureSubscription(UUID tenantId, String tenantType) {
        var planId = tenantRepository.findPlanIdByType(tenantType).or(tenantRepository::findAnyActivePlanId);
        if (planId.isEmpty()) return; // Sem planos cadastrados ainda — não faz nada
        tenantRepository.insertSubscriptionIfMissing(tenantId, planId.get());
    }
    ...
    public IndividualTenantResponse ensureIndividualTenant(UUID userId) {
        var existing = tenantRepository.findIndividualTenantByUser(userId);
        if (existing.isPresent()) {
            var row = existing.get();
            return new IndividualTenantResponse(row.id(), row.name(), row.slug(), "individual", true);
        }

        String fullName = userProfileRepository.findFullName(userId).orElse(null);
    ...
    public List<MyTenantDto> listMyTenants(UUID userId) {
        return userTenantRepository.findAllByUser(userId).stream().map(r -> new MyTenantDto(
