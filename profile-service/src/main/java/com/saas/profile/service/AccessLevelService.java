...
public class AccessLevelService {
    ...
    public AvailableModulesResponse availableModules(UUID tenantId) {
        List<AccessLevelRepository.ModuleServiceRow> rows = accessLevelRepository.findAvailableModuleTree(tenantId);

        Map<String, ModuleBuilder> moduleMap = new LinkedHashMap<>();
        Map<String, GroupBuilder> groupMap = new LinkedHashMap<>();

        for (AccessLevelRepository.ModuleServiceRow row : rows) {
    ...
    private static class ModuleBuilder {
        ...
        ModuleBuilder(AccessLevelRepository.ModuleServiceRow row) {
    ...
    private static class GroupBuilder {
        ...
        GroupBuilder(AccessLevelRepository.ModuleServiceRow row) {
    ...
    public List<AccessLevelDto> listAccessLevels(UUID tenantId) {
        return accessLevelRepository.findAllByTenant(tenantId).stream().map(level -> {
            UUID levelId = UUID.fromString(level.id());

            List<AccessLevelPermissionDto> permissions = accessLevelRepository.findPermissions(levelId).stream()
                    .map(p -> new AccessLevelPermissionDto(p.id(), p.moduleId(), p.serviceId()))
                    .toList();

            List<String> adminPerms = accessLevelRepository.findAdminPermissionKeys(levelId);
            long memberCount = accessLevelRepository.countActiveMembers(levelId);
    ...
    public String createAccessLevel(UUID tenantId, AccessLevelRequest request) {
        ...
        String levelId = accessLevelRepository.insertAccessLevel(tenantId, request.name().trim(), description);
    ...
    public void updateAccessLevel(UUID tenantId, UUID alId, AccessLevelRequest request) {
        ...
        boolean updated = accessLevelRepository.updateAccessLevel(tenantId, alId, request.name().trim(), description);
        if (!updated) {
            throw new NotFoundException("Nível de acesso não encontrado");
        }

        accessLevelRepository.deletePermissions(alId);
        accessLevelRepository.deleteAdminPermissions(alId);
    ...
    public String updateStatus(UUID tenantId, UUID alId, String status) {
        ...
        boolean updated = accessLevelRepository.updateStatus(tenantId, alId, status);
        if (!updated) {
            throw new NotFoundException("Nível de acesso não encontrado");
        }
    ...
    private void insertPermissions(UUID levelId, UUID tenantId, List<String> serviceIds) {
        for (String serviceIdStr : serviceIds) {
            ...
            if (!accessLevelRepository.isServiceAvailableForTenant(serviceId, tenantId)) {
                throw new BadRequestException("Serviço não disponível para este perfil: " + serviceIdStr);
            }

            String moduleId = accessLevelRepository.findModuleIdForService(serviceId);
            accessLevelRepository.insertPermission(levelId, UUID.fromString(moduleId), serviceId);
        }
    }
    ...
    private void insertAdminPermissions(UUID levelId, List<String> keys) {
        for (String key : keys) {
            ...
            accessLevelRepository.insertAdminPermission(levelId, key);
        }
    }
}
