...
import com.saas.profile.dao.InvitationDAO;
...
import com.saas.profile.repository.EmailRepository;
...
@ApplicationScoped
public class InvitationNegocioImpl implements InvitationNegocio {
    ...
    public List<MemberDto> listMembers(UUID tenantId) {
        return invitationDAO.findActiveMembers(tenantId).stream()
    ...
    public void removeMember(UUID tenantId, UUID targetUserId, UUID requestingUserId, String requestingRole) {
        ...
        String targetRole = invitationDAO.findActiveMemberRole(tenantId, targetUserId)
        ...
        invitationDAO.deactivateMember(tenantId, targetUserId);
        ...
        userTenantDAO.bumpVersionForMember(targetUserId, tenantId);
    }
    ...
    public void changeMemberAccessLevel(UUID tenantId, UUID targetUserId, String accessLevelId) {
        ...
        if (invitationDAO.findActiveAccessLevelName(tenantId, alId).isEmpty()) {
        ...
        invitationDAO.updateMemberAccessLevel(tenantId, targetUserId, alId);
        userTenantDAO.bumpVersionForMember(targetUserId, tenantId);
    }
    ...
    public List<InvitationDto> listInvitations(UUID tenantId) {
        return invitationDAO.findAllByTenant(tenantId).stream()
    ...
    public SendInvitationResponse sendInvitation(UUID tenantId, String email, String accessLevelId, UUID invitedBy) {
        ...
        String accessLevelName = invitationDAO.findActiveAccessLevelName(tenantId, alId)
                .orElseThrow(() -> new BadRequestException("Nível de acesso inválido ou inativo"));

        if (invitationDAO.countActiveMemberByEmail(tenantId, normalizedEmail) > 0) {
            throw new BadRequestException("Este usuário já é membro da empresa");
        }

        if (invitationDAO.countPendingInvitation(tenantId, normalizedEmail) > 0) {
            throw new BadRequestException("Já existe um convite pendente para este e-mail");
        }

        String tenantName = invitationDAO.findTenantName(tenantId);

        var created = invitationDAO.insertInvitation(tenantId, invitedBy, normalizedEmail, alId);
    ...
    public void cancelInvitation(UUID tenantId, UUID invitationId) {
        if (!invitationDAO.cancelPendingInvitation(tenantId, invitationId)) {
    ...
    public InvitationPreviewResponse getInvitationPreview(String token) {
        var row = invitationDAO.findPreviewByToken(token)
    ...
    public AcceptInvitationResponse acceptInvitation(String token, UUID userId, String userEmail) {
        String inviteEmail = invitationDAO.findAcceptableInvitationEmail(token).orElseGet(() -> {
            if (!invitationDAO.invitationTokenExists(token)) {
        ...
        Object result = invitationDAO.callAcceptInvitationFunction(token, userId);
