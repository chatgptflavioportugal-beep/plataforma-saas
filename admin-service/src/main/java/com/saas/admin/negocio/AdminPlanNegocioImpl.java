...
@ApplicationScoped
public class AdminPlanNegocioImpl implements AdminPlanNegocio {
    ...
    // participantes que já entraram continuam normalmente (ver TrialCampaignAdminNegocio).
    // ----------------------------------------------------------------
    ...
    private void cancelTrialsForOldVersion(String oldPlanId, int newVersion, String actorUserId) {
        ...
        List<UUID> cancelledIds = trialCampaignAdminNegocio.cancelCampaignsForPlanVersion(
            oldPlanId, reason, UUID.fromString(actorUserId));

        for (UUID campaignId : cancelledIds) {
            auditNegocio.log(actorUserId, "trial_campaign.plan_version_replaced",
