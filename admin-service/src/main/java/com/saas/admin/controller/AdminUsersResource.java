...
import com.saas.admin.service.AdminUsersService;
...
public class AdminUsersResource {
    ...
    public Response listAdminUsers(
        ...
        List<AdminUserDTO> users = usersService.list(search, status, accessLevelId);
    ...
    public Response createAdminUser(CreateAdminUserRequest req) {
        ...
        if (accessLevelId != null && !accessLevelId.isBlank() && !usersService.isAccessLevelActive(accessLevelId))
            return Response.status(400).entity(Map.of("error", "Nível de acesso não encontrado ou inativo")).build();

        String normalizedEmail = email.trim().toLowerCase();
        Optional<AdminUserDAO.ExistingUserRow> existing = usersService.findExistingByEmail(normalizedEmail);
        ...
        String effectivePassword = (tempPassword != null && !tempPassword.isBlank())
                ? tempPassword.trim()
                : usersService.generateTempPassword();

        String newUserId = null;
        if (usersService.isSupabaseConfigured()) {
            try {
                newUserId = usersService.createSupabaseUser(normalizedEmail, fullName.trim(), effectivePassword);
            } catch (Exception e) {
        ...
        Optional<AdminUserCreatedDTO> created = usersService.finalizeCreate(
    ...
    public Response updateAdminUser(
        ...
        if (usersService.isSuperAdmin(id))
        ...
        if (accessLevelId != null && !accessLevelId.isBlank()) {
            if (!usersService.isAccessLevelActive(accessLevelId))
                return Response.status(400).entity(Map.of("error", "Nível de acesso não encontrado ou inativo")).build();
            updated = usersService.updateProfile(id, fullName.trim(), accessLevelId, adminAuth.currentUserId());
        } else {
            updated = usersService.updateProfile(id, fullName.trim(), null, adminAuth.currentUserId());
        }
    ...
    public Response updateStatus(
        ...
        int updated = usersService.updateActiveStatus(id, isActive, currentUserId);
        if (updated == 0) return Response.status(404).entity(Map.of("error", "Usuário não encontrado")).build();
    ...
    public Response resetPassword(
        ...
        Optional<AdminUserDAO.EmailRoleRow> found = usersService.findEmailAndRole(id);
        if (found.isEmpty())
        ...
        boolean supabaseConfigured = usersService.isSupabaseConfigured();
        ...
        String newPassword = usersService.generateTempPassword();

        try {
            usersService.resetSupabasePassword(id, newPassword);
        ...
        boolean emailSent = usersService.sendResetEmailAndAudit(userEmail, newPassword, sendPasswordEmail, id, adminAuth.currentUserId());
    ...
    public Response sendPasswordEmailEndpoint(
        ...
        Optional<String> userEmail = usersService.findAdminEmail(id);
        if (userEmail.isEmpty())
            return Response.status(404).entity(Map.of("error", "Usuário não encontrado")).build();

        boolean sent = usersService.sendPasswordEmail(userEmail.get(), password, context);
        if (!sent)
