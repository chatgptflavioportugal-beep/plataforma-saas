...
export function ContextSelectorPage() {
  ...
  const { data: tenants = [], isLoading } = useQuery({
    queryKey: ['context-selector-tenants'],
    queryFn: async () => {
  ...
  function selectContext(tenantId: string) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900">Selecione um contexto</h1>
          <p className="mt-1 text-sm text-gray-500">
            Olá, {profile?.full_name ?? 'usuário'}. Em qual ambiente deseja trabalhar?
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            ...
            {individual ? (
              <button
                onClick={() => selectContext(individual.tenant_id)}
              ...
              /* Opção de criar perfil individual — para usuários sem tenant individual */
              <button
            ...
            {businesses.length > 0 && (
              <div className="space-y-2">
                ...
                {businesses.map((ut) => (
                  <button
                    key={ut.tenant_id}
                    onClick={() => selectContext(ut.tenant_id)}
