 ...
 * ACTIVE, TRIAL ou TRIAL_CANCELLED vencida como expirada independente deste job —
 * este scheduler só mantém a coluna status correta para telas que exibem o valor
 * persistido (ex.: badge de status em SubscriptionsPage). Da mesma forma, a
 * disponibilidade de vagas de uma trial_campaign é sempre calculada ao vivo
 * (used_slots < max_slots) nas queries de seleção — esgotar as vagas não muda o
 * status armazenado, só as transições de data (SCHEDULED→ACTIVE, ACTIVE→CLOSED).
 */
@ApplicationScoped
