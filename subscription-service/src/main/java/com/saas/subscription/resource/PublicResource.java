 ...
 * leitura aqui é SELECT-only (PlanNegocio); nenhum CRUD administrativo de
...
public class PublicResource {
    ...
    public Response listPlans(
        ...
        return Response.ok(planNegocio.listActivePlans(planType, page, size)).build();
    }
    ...
    public Response listModuleBillingOptions(
        ...
        @Parameter(description = "Tamanho da página, opcional. Só tem efeito quando `page` também é informado.")
        @QueryParam("size") Integer size) {
        return Response.ok(planNegocio.listModuleBillingOptions(page, size)).build();
    }
}
