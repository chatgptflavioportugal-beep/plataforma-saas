...
def mock_repository():
    ...
    with (
        patch("modules.pdf.repository.pdf_jobs_repository.create_job", new=AsyncMock(return_value=created)),
        patch("modules.pdf.repository.pdf_jobs_repository.complete_job", new=AsyncMock(return_value=completed)),
        patch("modules.pdf.repository.pdf_jobs_repository.fail_job", new=AsyncMock()),
        patch("modules.pdf.repository.pdf_jobs_repository.list_jobs", new=AsyncMock(return_value=[created])),
    ):
        yield
...
def test_merge_success(client):
    ...
    token = make_module_token(permissions=["module.pdf.pdf-merge"])
