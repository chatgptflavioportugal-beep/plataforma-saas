...
from negocio.merge_processor import process_merge
from negocio.merge_response import build_response
...
async def _validate_daily_limit(auth: ModuleContext) -> None:
    ...
    if max_daily is None:
        return
    done_today = await pdf_jobs_dao.count_jobs_today(auth.tenant_id)
...
async def execute(file_a: UploadFile, file_b: UploadFile, auth: ModuleContext) -> PdfJobOut:
    ...
    job = await pdf_jobs_dao.create_job(
        job_id=job_id,
    ...
    try:
        ...
        await pdf_jobs_dao.fail_job(str(job["id"]), str(e))
    ...
    job = await pdf_jobs_dao.complete_job(str(job["id"]), result_path, result_name)
