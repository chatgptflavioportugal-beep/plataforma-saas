...
import com.saas.auth.service.ProfileTokenService;
...
public class ProfileTokenResource {
    ...
    public Response generate(@Context SecurityContext secCtx) {
        ...
        var response = profileTokenService.issueAccessToken(ctx.getUserId(), ctx.getTenantId(), ctx.getUserRole());
    ...
    public Response permissionsVersion(@Context SecurityContext secCtx) {
        var ctx = TenantContext.from(secCtx);
        int version = profileTokenService.permissionsVersion(ctx.getUserId(), ctx.getTenantId());
