 ...
 */
@ApplicationScoped
public class ModuleTokenNegocioImpl implements ModuleTokenNegocio {
    ...
    @Inject
    ModuleServiceDAO moduleServiceDAO;
    ...
    public ModuleTokenResult issue(UUID userId, UUID tenantId, String role,
        ...
        ModuleAccessResolution access = subscriptionServiceRepository.resolveModuleAccess(
    ...
    private List<String> loadModulePermissions(UUID userId, UUID tenantId, UUID moduleId, String role) {
        if (List.of("owner", "admin").contains(role)) {
            return new ArrayList<>(moduleServiceDAO.findActiveServiceSlugsByModule(moduleId));
        }
        return new ArrayList<>(moduleServiceDAO.findServiceSlugsByAccessLevel(userId, tenantId, moduleId));
    }
}
